...
## 経費入力の自動化
- 決まったフォーマットの経費精算シート（例: 小学館 外部協力者用シート）がある場合、
  テンプレート＋スクリプトで明細を渡すだけで完成版を自動生成できる
- 仕組みとツールの詳細は `references/expense-automation.md` を参照
- レシート画像を受け取ったら日付・支払先・金額・税率（8%/10%）・インボイス番号の
  有無を読み取り、明細に変換する
- 交通費は固定パターン（例: 出社の往復）を既定化し、日付を並べるだけで入力できるようにする
